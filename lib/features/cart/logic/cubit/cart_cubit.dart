...
class CartCubit extends Cubit<CartState> {
  ...
  Future<void> emitRemoveFromCart(Map<String, dynamic> cartCourseData) async {
    emit(CartLoading());

    try {
      final response = await cartRepo.removeFromCart(cartCourseData);

      emit(RemoveCartSuccess(response.cart));
    } catch (e) {
